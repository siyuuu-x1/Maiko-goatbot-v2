    ...
    return async function (event, message) {
        ...
        const { body, messageID, threadID, isGroup } = event;
          ...
          if (event.type === "message_reaction") {
              const { reaction, messageID: reactMsgID } = event;
              const emojiReactBotSms = config.emojiReactBotSms || ["😡", "😠", "🤬"];
              if (emojiReactBotSms.includes(reaction)) {
                  api.unsendMessage(reactMsgID);
              }
          }
